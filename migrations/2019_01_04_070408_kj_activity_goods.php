...
class KjActivityGoods extends Migration
{
    ...
    public function up()
    {
        Schema::create('kj_activity_goods', function (Blueprint $table) {
            $table->increments('id');
            $table->integer('coupon_id')->comment('卡卷id');
            $table->string("goods_title")->comment('商品标题');
            $table->decimal("goods_price",10,2)->comment('商品价格');
            $table->integer("goods_num")->comment('商品库存');
            $table->integer("kj_goal")->comment('砍价目标');
            $table->integer("kj_num")->comment('砍价次数');
            $table->text('goods_images')->comment('商品图片');
            $table->text('goods_info')->comment('商品详情');
            $table->integer("activity_id")->comment('关联活动id');
